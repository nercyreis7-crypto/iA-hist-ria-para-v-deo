...
async def gerar_video_final(tema: str, fala: str, x_api_key: str = Header(...)):
    # 1. Limpeza de caracteres indesejados
    tema_limpo = tema.replace("#", "")
    fala_limpa = fala.replace("#", "")
    
    # 2. Configuração da API por requisição
    try:
    ...
    # 3. Geração do roteiro usando o tema e a fala personalizada
    try:
        prompt_ia = f"Crie um roteiro de 30s sobre {tema_limpo}. O personagem deve incluir esta fala: '{fala_limpa}'"
